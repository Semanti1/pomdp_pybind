...
from objectsearch.agent.agent import *
from objectsearch.env.visual import *

from objectsearch.example_worlds import *
from objectsearch.domain.observation import *
from objectsearch.models.components.grid_map import *
    ...
    def __init__(self, robot_id, env=None, grid_map=None,
        ...
        if env is None:
            ...
            env = MosEnvironment(dim,
                          ...
                          obstacles=obstacles)
            if env is None:
                print("Env still None after assign")
        if env is None:
                print("Env still None. Not sticking")
        # construct prior
        if type(prior) == str:
            if prior == "uniform":
                prior = {}
...
def solve(problem,
    ...
    if isinstance(random_object_belief, Histogram):
        # Use POUCT
        '''planner = POUCT(max_depth=max_depth,1,4096,
                                 discount_factor=discount_factor,1.4,
    ...
    for i in range(max_steps):
        ...
        #if isinstance(planner, POUCT):
         #   print("__num_sims__: %d" % planner._last_num_sims)
        ...
        # Termination check
        if set(problem.env.getstate().object_states[robot_id].objects_found)\
